...
func claudeCodeProbe(client *http.Client) Probe {
	return func(ctx context.Context, now time.Time) (*protocol.AgentQuota, error) {
		...
		accountUUID, err := claudeFetchAccountUUID(ctx, client, baseHeaders)
		if err != nil {
			return tombstone(now, "claude-code", "claude-code-oauth", "couldn't read profile: "+truncate(err.Error(), 200)), nil
		}
		if accountUUID == "" {
			return tombstone(now, "claude-code", "claude-code-oauth", "profile returned no account uuid"), nil
		}
...
func claudeFetchAccountUUID(ctx context.Context, client *http.Client, headers map[string]string) (string, error) {
	status, body, err := httpGetJSON(ctx, client, "https://api.anthropic.com/api/oauth/profile", headers)
	if err != nil {
		return "", err
	}
	if status >= 400 {
		return "", fmt.Errorf("profile %d: %s", status, truncate(string(body), 160))
	}
	var p struct {
		Account struct {
	...
	if err := unmarshal(body, &p); err != nil {
		return "", fmt.Errorf("profile unparseable: %w", err)
	}
