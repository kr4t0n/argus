...
slot, attributed to whichever project was busiest in it, plus the set of
projects with a turn running right now. Projects come back ranked, with no
colours — palettes are the renderer's call. Aggregates only (no titles,
prompts, or paths), so it is safe to re-serve publicly — do that through your
own backend holding an API key, not by shipping a credential to a browser.
